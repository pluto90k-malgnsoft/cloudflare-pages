...
### B. 타임라인 무결성 (Monotonicity) 체크 (v2.0 최우선 검증)
- **배경**: 안드로이드 MediaCodec/MSE는 타임스탬프의 단조 증가를 엄격히 요구함. 시간이 거꾸로 가면 지터가 아무리 낮아도 디코더가 정지함.
- **로직**:
...
- **효과**: "차가 흔들리는가(지터)" 이전에 "차가 바른 방향으로 가는가(타임라인)"를 먼저 검증하여 재생 불가 원인을 100% 진단.
...
- **3대 분석 지표**:
  ...
  3. **Intra-segment Audio PTS StdDev**: 세그먼트 내 오디오 PES 패킷 간 PTS 간격의 표준편차. 정상: <5ms, 이상: >5ms.
- **GAP 면역**: GAP/타임라인 역행 세그먼트는 aresample 분석에서 제외하여 소스 결함과 인코딩 옵션 미적용을 분리 판정.
