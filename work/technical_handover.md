# HLS Continuum Validator: 기술 아키텍처 및 알고리즘 핸드오버 (v2.2)
...
### B. 타임라인 무결성 (Monotonicity) 체크 (v2.0→v2.2 개선)
- **배경**: 안드로이드 MediaCodec/MSE는 연속 구간 내 타임스탬프의 단조 증가를 요구함.
- **v2.0 로직**:
  1. **Backward Jump 감지**: 이전 세그먼트 대비 현재 세그먼트의 시작 시간이 0.1초 이상 과거인 경우 포착.
  2. **Huge GAP 감지**: 데이터 간격이 10초를 초과하는 거대 GAP을 질적 결함으로 분류.
  3. **우선순위 엔진**: 타임라인 위반 발견 시 진동/오차 분석 결과를 무시하고 최우선 `FATAL` 리포트.
- **v2.2 개선 — 콘텐츠 경계 인식**:
  - **문제 발견**: PTS 역행이 VOD 편집/연결점에서 정상적으로 발생하는 경우가 있음. `#EXT-X-DISCONTINUITY` 태그 없이도 PTS가 역행하지만 안드로이드 크롬에서 정상 재생됨.
  - **해결**: PTS 역행이 동반된 GAP을 **콘텐츠 경계**로 분류하여 FATAL 판정에서 제외.
  - **판정 기준**: GAP 없이 연속 구간 내에서 발생하는 역행만 진짜 `FATAL: TIMELINE REVERSAL`로 판정.
  - **UI**: 콘텐츠 경계 세그먼트에 `DISC` 배지(파란색), 파란 배경 행 스타일 적용.
- **효과**: 편집/연결된 VOD 콘텐츠의 오탐(false positive)을 제거하면서 진짜 타임라인 결함만 포착.

### C. 영점 교차율 (Zero-crossing Rate, ZCR) 분석 (v1.9 GAP 면역 체계)
...
- **3대 분석 지표**:
  1. **Audio Continuity Gap**: 세그먼트 N의 마지막 오디오 PTS → 세그먼트 N+1의 첫 오디오 PTS 간 간격. 정상: <150ms(AAC 6프레임 이내), 이상: >150ms.
  2. **Duration Mismatch**: 세그먼트 내 오디오 실제 duration(`lastPTS - firstPTS`) vs m3u8 `#EXTINF` 선언 duration. 정상: <100ms 편차, 이상: >100ms.
  3. **Intra-segment Audio PTS StdDev**: 세그먼트 내 오디오 PES 패킷 간 PTS 간격의 표준편차. 정상: <10ms, 이상: >10ms.
- **GAP 면역**: GAP/콘텐츠 경계 세그먼트는 aresample 분석에서 제외하여 소스 구조와 인코딩 옵션 미적용을 분리 판정.
- **판정 로직**: 정상 연속 세그먼트에서 3개 지표 중 하나라도 임계값 초과 시 `aresample=async=1 미적용 의심` 판정.
- **실측 검증**: `V22133.m3u8` (aresample 적용, 정상 재생) 분석 시 "적용됨" 판정 확인.
